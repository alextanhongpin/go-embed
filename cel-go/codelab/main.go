...
func exercise1() {
	...
	if !proto.Equal(checked.ResultType(), decls.String) {
		log.Fatalf("got %v, wanted %v result type", checked.ResultType(), decls.String)
	}
...
func exercise2() {
	...
	// Evaluate a request object that sets the proper group claims.
...
func exercise3() {
	...
	ast := compile(env, `request.auth.claims.group == 'admin' || request.auth.principal == 'user:me@acme.co'`, decls.Bool)
	program, _ := env.Program(ast)
...
func exercise4() {
	...
	// Declare the parameterized map types used for `contains` function
	// signature.
	...
	ast := compile(env, `request.auth.claims.contains('group', 'admin')`, decls.Bool)
	...
	) // Add the custom function binding here with cel.Functions().
	...
	eval(
		program,
		request(auth("user:me@acme.co", claims), time.Now()),
	)
